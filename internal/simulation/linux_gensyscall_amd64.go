...
type linuxOSIface interface {
	...
	SysNewfstatat(fd int, path string, stat syscallabi.ValueView[Stat_t], flags int) (err error)
...
func SyscallSysNewfstatat(fd int, path string, stat *Stat_t, flags int) (err error) {
	// invokes (for go to definition):
	_ = (*LinuxOS).SysNewfstatat
	syscall := syscallabi.GetGoroutineLocalSyscall()
	syscall.OS = linuxOS
	syscall.Trap = unix.SYS_NEWFSTATAT
	syscall.Int0 = uintptr(fd)
	syscall.Ptr1 = path
	syscall.Ptr2 = stat
	syscall.Int3 = uintptr(flags)
